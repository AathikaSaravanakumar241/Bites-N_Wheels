...
export function CartProvider({ children }) {
    ...
    get('/api/v1/cart')
      .then((data) => {
        if (data && Array.isArray(data.items)) {
          setCart({ truckId: data.truckId ?? null, items: data.items })
        }
      })
      .catch(() => {})
  }, [])
  ...
  const addItem = useCallback(async (truckId, itemId) => {
    ...
    try {
      ...
      setCart((c) => ({ ...c, truckId, items: updated }))
    } catch (err) {
  ...
  const placeOrder = useCallback(async ({ schedule, payment, note }) => {
    ...
    setOrders((list) => [{ ...data, lines }, ...list])
    clearCart()
    return orderId
  }, [cart.truckId, lines, clearCart])
